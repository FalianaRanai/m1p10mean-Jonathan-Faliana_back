...
exports.updateEmploye = async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const { nomEmploye, prenomEmploye, user, email, password } = req.body;

        verifyArgumentExistence(["id"], req.params);
        verifyArgumentExistence(["nomEmploye", "prenomEmploye", "user", "email", "password"], req.body);
        ...
        Userdb.findByIdAndUpdate(new ObjectId(user), newUserData, {
            session,
        })
            .then(() => {

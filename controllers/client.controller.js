...
exports.getClient = (req, res) => {
  ...
  try {
    ...
    Clientdb.find({ _id: id, isDeleted: false })
      ...
      .populate({
        path: "preference",
        populate: [{ path: "service" }, { path: "employe" }],
      })
      .populate({
        path: "historiqueRDV",
        populate: [
...
exports.getRappelRendezVous = (req, res) => {
  const functionName = "getRappelRendezVous";
  try {
    const { client } = req.params;
    verifyArgumentExistence(["client"], req.params);
    ...
    Rendezvousdb.find({
      ...
      dateDebutRdv: { $gte: moment(d1).tz("Indian/Antananarivo") },
      dateFinRdv: { $lte: moment(d2).tz("Indian/Antananarivo") },
    })
      .populate({
        path: "client",
        populate: [

...
exports.addRendezvous = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < listeTaches.length; i++) {
      ...
      // let service = await Servicedb.findOne({ _id: new ObjectId(tache.service) });
      let service = await RdvController.findServiceDetails(
        { isDeleted: false, _id: new ObjectId(tache.service) },
        new Date(),
        new Date()
      );
      ...
      temp.push({
        ...tache,
        prix: service[0].prix,
        prixAvantRemise: service[0].prixAvantRemise,
      });
    }
    ...
    for (let i = 0; i < updateListeTachesEmploye.length; i++) {
      await Employedb.updateOne(
        ...
        {
          $push: {
            listeTaches: new ObjectId(updateListeTachesEmploye[i].idTache),
          },
        }
      );
    }
    ...
    dataToInsert
      ...
      .then(async (data) => {
        ...
        await sendMail(
          ...
          "Paiement effectué avec succès le " +
            moment().tz("Indian/Antananarivo").format("YYYY-MM-DD HH:mm:ss"),
          "Votre paiement a été réalisé avec succès le " +
            moment().tz("Indian/Antananarivo").format("YYYY-MM-DD HH:mm:ss") +
            ".",
          htmlTask
        );
...
exports.getRdvReservationParJour = (req, res) => {
  ...
  try {
    ...
    Rendezvousdb.aggregate(pipeline)
      .then((result) => {
